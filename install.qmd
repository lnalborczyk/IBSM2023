...
Le reste des paquets ainsi que les données utilisées dans les exercices, et quelques fonctions utiles, peuvent être installées ensemble sous la forme d'un paquet "compagnon" de la formation, le paquet `imsb`. Pour l'installer, vous devez d'abord installer le paquet `remotes` (et `tidyverse` au passage), puis installer `imsb` depuis Github.^[Yeah, the `imsb` acronym comes from the French name of the course I am teaching, hence the loose correspondence with this course's acronym.]
