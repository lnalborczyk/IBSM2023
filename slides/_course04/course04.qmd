...
priority of $\alpha_{\text{café}}$ ($\sigma_{\text{café}} = 0.82$). Note
that this sigma is very low is very low and corresponds to assigning a
very restrictive or **regularising** prior.

## Model comparaison

The estimates of the first model (complete pooling model) and the third
model (partial pooling model) are compared.
...
Note: we know that cafés have intercepts and slopes which co-vary...
Popular cafés will be overcrowded in the morning and much less in the
afternoon, resulting in a significant slope. These cafés will also have
...
difference between the morning and afternoon's waiting the.
...
coefficient. The $\mathrm{LKJ}(2)$ prior defines an uninformative prior
...
The first model above contains only an intercept variable, which varies
by `Subject`. The second model also contains a variable intercept, but
also a variable slope for the effect of `Days`.
...
these two effects. Otherwise, we can remove this correlation (i.e., set
it to 0) using `||`.
...
-   The **boundary separation** $\alpha$ (alpha) is the distance between
    ...
    response caution, with high $\alpha$ meaning high caution.
...
We will adapt the example from @singmann_diffusion/wiener_2017. We will
analyse part of the data from Experiment 1 of @wagenmakers2008. The data
comes from 17 participants performing a lexical decision task in which
they have to decide if a presented string is a word or non-word.
Participants made decisions either under speed or accuracy emphasis
...
common constraint of the DDM is that the parameters that are set before
...
of the trial. Thus, the item-type, in the present case word versus
...
within-subject. Thus, the maximal random-effects structure
[@barr_random_2013-1] entails corresponding random-effects parameters
...
We then fit this model using the `brms::brm()` function. We run 8
chains, each for 5000 iterations and using the first 1000 iterations
used as warmup, resulting in a total of $8 \times (5000 - 1000) = 32000$
posterior samples.
...
This plot shows that words are recognised faster than non-words, that
incorrect responses are faster than correct responses, and the responses
are generally faster in the "speed" than in the "accuracy" condition.
